<template>
  <view class="login-layout">
    <view class="form-contend">
      <view class="logo"><image src="../../static/A-CDM.png" mode="aspectFit"></image></view>
      <view class="input">
    ...
    <!-- <view class="bottom" :style="{top: (forgetHeight - 100) + 'px'}">
			<text>忘记密码？</text>
		</view> -->
    <view class="ident-class">
      <text>指纹/面容登录</text>
      <switch @tap="identTap" :disabled="judgeIdent" @change="switchChange" />
    </view>
    <view class="vision">
...
export default {
  ...
  methods: {
    ...
    loginVPN (flag) {
      ...
      // const vpn_address2 = 'https://59.60.12.50'
      // const vpn_address3 = 'https://183.250.160.212'
      const vpn_address = ['https://vpn.iport.com.cn', 'https://59.60.12.50', 'https://183.250.160.212']
      this.loginToVpn(flag, vpn_address[this.currentAddressIndex])
    },
    loginToVpn (flag, vpn_address) {
      console.log('vpn_address22', vpn_address)
      ...
      let that = this
      this.$loading(this.currentAddressIndex > 0 ? '切换地址登入中...' : '登入中...')
      uni.hideKeyboard()
      let time = 0
...
.login-layout {
  ...
  width: 100vw;
  overflow-x: hidden;
  // background: linear-gradient(#1890ff, white);
  padding: 0 30rpx;
  overflow: hidden;
  ...
  .form-contend {
    > .logo {
      padding: 150rpx 220rpx 100rpx 220rpx;
