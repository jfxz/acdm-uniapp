	...
	<view class="home-layout">
		...
		<view class="home-bottom" :style="{ height: bottomHeight + 'px' }">
			<view class="item" v-for="(item, index) in items" @tap="jump(item)" :Key="index">
				<view class="item-img" v-if="item.jumpUrl!=='ferry'">
...
<script>
	...
	export default {
		...
		onLoad(e) {
      uni.request({
        url: 'https://vpn.iport.com.cn',
        complete: function(res) {
          if (res.data) {
            uni.setStorageSync('vpnDeviceStatus', '成功')
          } else {
            uni.setStorageSync('vpnDeviceStatus', '失败')
          }
        }
      })
