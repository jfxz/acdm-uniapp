		...
		mounted () {
			...
			if (this.leftVersionList && this.rightVersionList) {
				for (let i = 0; i < this.leftVersionList.length; i++) {
					...
					let rightObj = this.rightVersionList.find(e => e.container === leftObj.container)
					if (rightObj) {
						rightbp.push({
							index: i,
							...rightObj
				...
				for (let i = 0; i < this.rightVersionList.length; i++) {
					...
					if (!rightbp.find(e => e.container === rightObj.container)) {
			...
			initList (res) {
				...
				for (let key in res.data) {
					if (res.data[key] && res.data[key].length > 0) {
						this.flight = res.data[key][0].flight
					}
