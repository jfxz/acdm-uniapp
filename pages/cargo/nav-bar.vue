				...
				<slot name="right">
					<view slot="right" v-if="history">
						<view class="operation-list">
							<template v-if="showCloseOff">
								<view :class="{'item': true, distance: outStock, 'right': outStock}" style="margin-right: 20rpx" @tap="openMessages">
									<view style="width:64rpx;height:54rpx;position: relative;">
										<image style="width:54rpx;height:53rpx;" src="@/static/small-bell.png"></image>
										<u-badge
											is-center
											size="mini"
											bgColor="#ffffff"
							...
							<template v-if="outStock">
								<view :class="{'item': true, distance: outStock, 'right': outStock}" style="margin-right: 20rpx" @tap="openOutStockCloseOff">
									<image style="width:54rpx;height:53rpx;" src="@/static/out-stock-closeoff.png"></image>
								</view>
							</template>
							...
							<view v-if="showSearchFlightBtn" :class="{'item': true, distance: showOperationHistory||showSort, 'right': !showOperationHistory&&!showSort}">
								<image style="margin-left:10rpx;height:44rpx;" src="@/static/operation-history-search.png" @tap="searchFlight"></image>
							</view>
							...
							<template v-if="showRefreshBtn">
								<view :class="{'item': true, distance: showRefreshBtn, 'right': showRefreshBtn}" style="margin-right: 30rpx" @tap="refreshList">
									<image style="width:54rpx;height:53rpx;" src="@/static/refresh.png"></image>
								</view>
							</template>
							
							<template v-if="showCloseOffScanBtn">
								<view :class="{'item': true, distance: showCloseOffScanBtn, 'right': showCloseOffScanBtn}" style="margin-right: 30rpx" @tap="closeoffScan">
									<image style="width:54rpx;height:53rpx;" src="@/static/scan-cargo-sm.png"></image>
								</view>
							</template>
		...
		<template v-if="!contentNotNull">
			...
			<template v-if="!showSearchBar">
				<view class="operation" style="margin-top: -1rpx;padding-top:1rpx"  v-if="history">
					<view class="item left" :class="{'out-stock-left': outStock}">
						<image style="width:51rpx;height:53rpx;" src="@/static/scan-cargo-lg.png" @tap="scanCode"></image>
						<view class="font">扫一扫</view>
					</view>
					
					<view :class="{'item': true, 'right': !showOperationHistory&&!showSort&&!showSearchFlightBtn}">
					...
					<view v-if="showSearchFlightBtn" :class="{'item': true, 'right': !showOperationHistory&&!showSort}" @tap="searchFlight">
						<image src="@/static/operation-history-search.png"></image>
						<view class="font">查询航班</view>
					</view>
					...
					<template v-if="showOperationHistory">
						<view :class="{'item': true, 'right': !outStock&&showOperationHistory}" @tap="openOperaionHistory">
			...
			<template v-else>
				<view class="operation-search" style="margin-top: -1rpx;padding-top:1rpx">
					<view class="operation-above-search" v-if="history">
						<template v-if="outStock">
							<view :class="{'item': true, distance: outStock, 'right': !outStock}" style="margin-left: 20rpx;">
								<image style="width:43rpx;height:42rpx;" src="@/static/out-stock-closeoff.png" @tap="openOutStockCloseOff"></image>
							</view>
						</template>
						...
						<view v-if="showSearchFlightBtn" :class="{'item': true, distance: showOperationHistory||showSort, 'right': !showOperationHistory&&!showSort}">
							<image src="@/static/operation-history-search.png" @tap="searchFlight"></image>
						</view>
					...
					<view class="search" v-if="history">
						<view class="search-input">
							<input :focus="focus" @input="inputSearch" confirm-type="search" @confirm="inputGetCargoInfo" style="width: 92%;" :placeholder="placeHolder" v-model="inputValueInner"/>
	...
	export default {
		...
		data () {
			return {
				...
				timer: null,
				customBack: function () {
					console.log('customBack-0')
					if (this.jumpFromDriver) {
		...
		methods: {
			// 查看消息
			openMessages: debounce(function () {
				this.$emit('openMessages')
			}),
			// 出库截关
			openOutStockCloseOff: debounce(function () {
				uni.navigateTo({
					url: `/pages/cargo/leaveport/out-stock-closeoff`
				})
			}),
			inputSearch(item) {
        if (this.placeHolder !== '请输入航班号') {
          this.$emit('inputSearch', item.detail.value)
        }
			},
			// 隐藏/显示 搜索栏
			changeSearchBarStatus: debounce(function () {
				this.$emit('changeSearchBarStatus', '请输入集装器号')
			}),
			// 扫码
			scanCode () {
				this.$emit('scanCode')
			...
			// 按下搜索栏确定按钮录入集装器
			inputGetCargoInfo: debounce(function (item) {
				if (this.showSearchFlightBtn && this.placeHolder.indexOf('航班') !== -1) {
					this.$emit('searchFlight', this.inputValueInner)
				} else {
					this.$emit('inputGetCargoInfo', this.inputValueInner)
				}
				if (this.clearAfterInput) {
			...
			// 显示列表的选择框
			showSelectFlag: debounce(function () {
				this.$emit('showSelectFlag')
			}, 300),
			// 打开操作历史
			openOperaionHistory: debounce(function () {
				this.$emit('openOperaionHistory')
			}, 300),
			
			deleteList: debounce(function () {
			...
			searchFlight: debounce(function () {
				this.$emit('changeSearchBarStatus', '请输入航班号')
			}),
		}
	}
</script>
