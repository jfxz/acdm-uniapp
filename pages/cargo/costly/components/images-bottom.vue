<template>
	<view class="bottom-image" v-if="show">
			...
			<view @touchstart="changeSwipeStatus" @touchend="changeSwipeStatus" class="images">
				<block v-for="image in item.images" :key="image.name">
					<view class="image-layout" v-if="image.show">
						<image :src="image.uri" @tap.stop="showImage(image)"></image>
...
<script>
	import {deleteImage} from '@/api/cargo/index.js'
	...
	export default {
		...
		methods: {
			...
			deleteImage (item, image) {
				if (item.createBy && item.createBy !== this.cargoUserForm.username) {
					...
					deleteImage(removedImage).then(res => {
						if (res.status) {
							...
							if (item.specialCargoRecords && item.specialCargoRecords.length !== 0) {
								...
								// 每个步骤的三张图片共用一个id
								let step = item.specialCargoRecords.find(e => e.id === removedImage.id)
								if (step) {
									// 如果当前步骤之前有上传过照片
									...
									if (imageArr.length > 1) {
										...
										// 如果当前步骤之前只上传过一张照片，那么删除该照片后直接在照片属性specialCargoRecords中删除该步骤即可
										let stepIndex = -1
										stepIndex = item.specialCargoRecords.findIndex(e => e.id === removedImage.id)
										if (stepIndex > -1) {
											item.specialCargoRecords.splice(stepIndex, 1)
										}
									}
								}
							}
							// 如果上面的if没执行，说明这个卡片之前都没上传过图片，直接删除本地图片即可。如果执行了。在删除specialCargoRecords中的图片之后还要删除本地图片
