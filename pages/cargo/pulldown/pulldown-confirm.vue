						...
						<block v-for="item in list" :key="item.id">
							...
							<view class="item" style="margin-top: 10rpx;" @tap="checkPlateBox(item)">
								...
								<view class="item-padding-right">
									<unloading-flight-card :item-ori="item"></unloading-flight-card>
								</view>
							</view>
						</block>
	...
	export default {
		...
		methods:{
			...
			checkPlateBox: debounce(function (item) {
				...
				uni.navigateTo({
					url: `/pages/cargo/pulldown/pulldown-confirm-box`
				})
			}),
			selectFlightList () {
				this.$loading()
				this.loadStatus = 'loading'
				getPulldownConfirmFlights(this.selectFlightForm).then(res => {
					if (res.status) {
						this.flightList = res.data
						this.flightList.sort((a, b) => {
							if (a.flightStatus && a.flightStatus !== '') {
								return -1
							} else {
								return 1
							}
						...
						this.list.forEach(e => {
							e.opDate = dayjs(e.opDate).format('YYYY-MM-DD')
						})
			...
			loadmore () {
				if (!this.endFlag) {
					...
					let next = this.flightList.slice(start, end)
					next.forEach(e => {
						e.opDate = dayjs(e.opDate).format('YYYY-MM-DD')
					})
