	...
	export default {
		...
		onLoad (e) {
			if (e.hasOwnProperty('history') && e.history === 'true') {
				...
				if (!this.completedPulldownToStation) {
					let list = uni.getStorageSync('completedPulldownToStation')
					if (list) {
						this.list = [...list]
						var that = this
						this.$nextTick(function(){
		...
		onBackPress () {
			...
			if (!this.completedPulldownToStation) {
				uni.setStorageSync('completedPulldownToStation', this.list)
			}
		},
		...
		methods:{
				...
				if (!this.showOperationHistory) {
					...
					addPulldownFromBatch(addForm).then(res => {
						...
						if (res.status) {
							this.scanSound()
							uni.vibrateLong()
							let newObj = res.data.find(v => v.containerCode === addForm.uld)
							...
							this.setOperationHistoryList([obj, ... this.list])
							this.list = this.operationHistoryList
					...
					getPulldownCargoInfo(this.cargoInfoForm).then(res => {
						...
						if(res.status) {
							this.scanSound()
							uni.vibrateLong()
							this.setCompletedPulldownToStation(false)
			...
			confirmBtn () {
				...
				pulldownConfirm(this.confirmForm).then(res => {
					console.log('pulldownConfirm', res)
					this.$message(res.message)
					this.setCompletedPulldownToStation(true)
					uni.removeStorageSync('completedPulldownToStation')
					uni.hideLoading()
				}).catch(err => {
					onError(err)
					uni.hideLoading()
			...
			showSelectFlag () {
				...
				if (!this.history) {
					uni.navigateTo({
						url: `/pages/cargo/sort-page?operationType=TOSTATION_STEVEDORING&direction=PULL&pageName=to-cargo-station-stevedoring&portDirection=PULL`
					})
				} else {
					uni.navigateTo({
						url: `/pages/cargo/sort-page?operationType=TOSTATION_STEVEDORING&direction=PULL&pageName=to-cargo-station-stevedoring&portDirection=PULL&history=true`
					})
				}
			},
			openOperaionHistory () {
