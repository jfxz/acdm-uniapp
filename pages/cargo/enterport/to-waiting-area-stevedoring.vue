...
<script>
	...
	export default {
		...
		onShow () {
			if (this.backFlag) {
				...
				if (list) {
					...
					console.log('sort-listbe-before', this.list)
					console.log('sort-listbe-after', list)
					list.forEach(e => {
		...
		onBackPress () {
			...
			if (!this.completedEnterToWaitingArea) {
				uni.setStorageSync('completedEnterToWaitingArea', this.list)
			}
		},
		onLoad (e) {
			if (e.hasOwnProperty('history') && e.history === 'true') {
				...
				if (!this.completedEnterToWaitingArea) {
					let list = uni.getStorageSync('completedEnterToWaitingArea')
					if (list) {
						this.list = [...list]
						var that = this
						this.$nextTick(function(){
		...
		methods:{
				...
				if (!this.showOperationHistory) {
					...
					getCargoInfo(this.cargoInfoForm).then(res => {
						...
						if(res.status) {
							this.scanSound()
							uni.vibrateLong()
							this.setCompletedEnterToWaitingArea(false)
			...
			showSelectFlag () {
				...
				if (!this.history) {
					uni.navigateTo({
						url: `/pages/cargo/sort-page?operationType=TOWAITINGAREA_STEVEDORING&direction=A&pageName=to-waiting-area-stevedoring&portDirection=enterport`
					})
				} else {
					uni.navigateTo({
						url: `/pages/cargo/sort-page?operationType=TOWAITINGAREA_STEVEDORING&direction=A&pageName=to-waiting-area-stevedoring&portDirection=enterport&history=true`
					})
				}
			},
			openOperaionHistory () {
			...
			confirmBtn () {
				...
				confirm(this.confirmForm).then(res => {
					...
					this.$message(res.message)
					this.setCompletedEnterToWaitingArea(true)
					uni.removeStorageSync('completedEnterToWaitingArea')
					uni.hideLoading()
					setTimeout(function () {
						uni.navigateBack()
					}, 1000)
