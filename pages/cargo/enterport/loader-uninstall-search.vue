	...
	export default {
		...
		methods: {
			...
			search (searchForm1) {
				...
				searchForm.flight = searchForm.flight.match(/\d+/g)[0]
				...
				getFlights(searchForm).then(res => {
					...
					if (res.status && !validatenull(res.data)) {
						...
						if (this.flights.length === 1) {
							// let word = this.flights[0].match(/[a-zA-Z]+/g)
							// getFilteredFlight({
							// 	iataCode: word
							// }).then(res => {
							// 	if (res.status && validatenull(res.data)) {
