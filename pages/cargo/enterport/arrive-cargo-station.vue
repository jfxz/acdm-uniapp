	...
	export default {
		...
		onLoad (e) {
			...
			if (e.uld) {
				...
				this.getCargoInfo()
			} else if (!this.completedEnterArrivedStation) {
				let list = uni.getStorageSync('completedEnterArrivedStation')
				if (list) {
					this.list = [...list]
		...
		onBackPress () {
			if (!this.completedEnterArrivedStation) {
				uni.setStorageSync('completedEnterArrivedStation', this.list)
			}
		},
		mounted () {
			this.cargoRemind('TOSTATION_DRIVER', '交接运往货站司机')
			var that = this
			this.$nextTick(function(){
				setTimeout(function(){
		...
		methods:{
			...
			getCargoInfo () {
				...
				getCargoInfo(this.cargoInfoForm).then(res => {
					...
					if(res.status && !validatenull(res.data)) {
						this.scanSound()
						uni.vibrateLong()
						this.setCompletedEnterArrivedStation(false)
						this.list = res.data
			...
			completedArrived (flag = true) {
				...
				this.setCompletedEnterArrivedStation(true)
				...
				uni.removeStorageSync('completedEnterArrivedStation')
			},
			
			selectAll () {
				this.$refs.listContent.selectAll()
				if (this.$refs.listContent.judgeAtLeastSelected('ARRIVEDSTATION')) {
					this.btnCanPress = true
				} else {
