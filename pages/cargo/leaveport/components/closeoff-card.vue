	...
	<view v-show="refresh">
		<view class="cardRight">
			<view class="top">
				...
				<view v-if="scheduleDepTime" class="center">
					<view class="label">时间</view>
					<view class="value">{{scheduleDepTime}}</view>
				</view>
				<view class="right">
					<view class="status">
...
<script>
	...
	export default {
		...
		computed: {
			scheduleDepTime () {
				if (!this.item.scheduleDepTime) {
					return ''
				}
				return dayjs(this.item.scheduleDepTime).subtract(1, 'hour').format('YYYY-MM-DD HH:mm')
			}
		},
		methods: {
			// 上传图片
