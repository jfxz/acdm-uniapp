	...
	<view class="cu-modal" :class="showModal==true?'show':''">
		<view class="cu-dialog" style="max-width:620rpx;border: 15upx;">
			<view class="action" @tap="showModal=false">
				<image
					class="action-image"
				  src="@/static/close-popup.png"></image>
			</view>
			<view class="title">
				装卸舱位录入
			</view>
					...
					<view style="display: flex;justify-content: center;width: 100%;">
						<view class="btns">
							<block v-for="item in cargoHolds" :key="item.name">
								<view :class="{'btn': !item.active, 'btn-selected': btnable&&item.active}" @tap="checkCargoHolds(item)">
									{{item.name}}
								</view>
							</block>
						</view>
	...
	export default {
		...
		methods: {
			open (item) {
				...
				if (item.secondCabin) {
					try {
						...
						if (!validatenull(obj.installedConfirmSecond)) {
							this.btnable = false
							this.confirmedInfo = obj
						}
					} catch (err) {
			...
			checkCargoHolds2: debounce(function (item) {
				console.log('item', this.currentContainer)
				this.cargoHolds.forEach(e => {
					e.active = false
				})
				item.active = true
			...
			checkCargoHolds: debounce(function (item) {
				var that = this
				if (!this.btnable) {
					let confirmedInfo = this.confirmedInfo
					let first = confirmedInfo.installedConfirmFirst
					let second = confirmedInfo.installedConfirmSecond
				...
				try {
					...
					// 装机确认已经确认的舱位实体
					selectedCabinObj = obj.installedConfirmFirst
				} catch (err) {
				...
				if (obj) {
					selectedCabinObj = obj.installedConfirmFirst
					selectedSecondObj = obj.installedConfirmSecond
				}
