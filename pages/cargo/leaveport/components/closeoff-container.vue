...
<template>
	<view>
		<view class="top">
			...
			<view class="right">
				<view class="status-right" :style="{'color':item.status=='NOTOUTOFSTOCK'?'red':'#2773e5'}">
					{{item.status=='ASSEMBLY'?'配上航班':formatStatus(item.status, item)}}
				</view>
			</view>
		</view>
		<view class="flight-place" style="margin-bottom: 0;">
			<view class="left">
				<view class="flight block">
					<view class="font">复称状态:</view>
					<view class="flight-number">{{formatterReweighStatus(item.reweighStatus)}}</view>
				</view>
				<view class="place block">
					<view class="font">库位:</view>
					<view class="place-font">{{item.locationCode}}</view>
				</view>
		...
		<view class="flight-place">
			<view class="left">
				<!-- <view class="flight block">
					<view class="font">出库人:</view>
					<view class="flight-number">{{item.stockPerson}}</view>
				</view>
				<view class="place block">
					<view class="font">出库日期:</view>
					<view class="place-font">{{item.stockedTime?item.stockedTime.substr(0, 10):''}}</view>
				</view> -->
				<!-- <view class="place block">
					<view class="font">类型:</view>
					<view class="place-font">{{containerType}}</view>
				</view> -->
			</view>
...
<script>
	...
	export default {
		...
		computed: {
			containerType () {
				if (this.item.containerType === '0') {
					return '板'
				} else if (this.item.contentType === '1') {
					return '箱'
				} else if (this.item.containerType === '2') {
					return '车'
				} else {
					return ''
				}
