    ...
    <view class="header">
      <view class="bg">
        <view class="box">
          ...
          <view class="box-bd cnabbr2w">{{ user.userAirport ? user.userAirport[0].cnabbr2w : '' }}</view>
        </view>
      </view>
    </view>
...
import {getUserMsg} from '../../api/user/index.js'
  ...
  methods: {
    ...
    loginOut () {
      if (this.userInfo.loginId) {
        exitLogin(this.userInfo.loginId).then(res => {
          if (res.status) {
            // 断开mq
            stomp.closeConnect()
            // 清除消息、用户缓存
    ...
    jump(path) {
      if (path === 'ip') {
        ...
        getUserMsg().then(res => {
          if (res.status) {
            this.userIp = res.data.meta.ip
            this.$message('当前ip为：' + this.userIp)
          }
          uni.hideLoading()
        })
        this.$message()
