<template>
  <view class="pass-layout">
    <view class="total">
      ...
      <view class="total-text">今年累计起飞正常率{{ formatFixed2Value(totalRate) }}%</view>
    ...
    <view class="narmal-charts" v-if="!topLoadFail">
...
export default {
  ...
  methods: {
    ...
    getmonthData () {
      let startMonth = Number(this.indexParams.showList.monthTakeOffRate)
      let endMonth = Number(this.indexParams.showList.monthTakeOffRate) + 11
      let para = {
      ...
      getDayAndMonthRate(para)
        .then(res => {
          if (res.status) {
            ...
            let markLineVal = this.indexParams.markLineVal[`${this.airportCode}monthTakeOffRate`]
            ...
            this.chartDataMonth = Object.assign({}, this.chartDataMonth, {
              ...
              series: [{ name: '月起飞正常率', data: val }],
              flag: this.indexParams.markLineFlag.monthTakeOffRate || false,
              markLineVal: this.indexParams.markLineVal[`${this.airportCode}monthTakeOffRate`] || 70
            })
      ...
      getDayAndMonthRate(para)
        .then(res => {
          if (res.status) {
            ...
            let markLineVal = this.indexParams.markLineVal[`${this.airportCode}dayTakeOffRate`]
            let val = []
            orginData.forEach((item, index) => {
              xAxisData.push(index + 1)
              let value = (item.normalRate * 100).toFixed(2)
            ...
            this.chartData = Object.assign({}, this.chartData, {
              categories: xAxisData,
              series: [{ name: '日起飞正常率', data: val }],
              flag: this.indexParams.markLineFlag.dayTakeOffRate || false,
              markLineVal: this.indexParams.markLineVal[`${this.airportCode}dayTakeOffRate`] || 70
            })
