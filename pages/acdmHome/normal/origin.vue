<template>
  <view class="pass-layout">
    <view class="total">
      ...
      <view class="total-text">今年累计始发起飞正常率{{ formatFixed2Value(totalRate) }}%</view>
...
export default {
  ...
  methods: {
    ...
    getmonthData () {
      let startMonth = Number(this.indexParams.showList.monthMultiple)
      let endMonth = Number(this.indexParams.showList.monthMultiple) + 11
      let para = {
        airportCode: this.airportCode,
        name: 'original_takeoff',
        frequency: 2
      }
        ...
        .then(res => {
          if (res.status) {
            ...
            this.chartDataMonth = Object.assign({}, this.chartDataMonth, {
              ...
              series: [{ name: '月始发起飞正常率', data: val }]
            })
            console.log('chartDataMonth', this.chartDataMonth)
